<script>
  ...
  // Fungsi untuk mengambil data dengan filter branch
  async function fetchData() {
    try {
      ...
      console.log(`🔄 Loading sales inquiry data for branch: ${userBranch} (${selectedFilter})...`);
      ...
      console.log(`⚡ Branch sales inquiry data loaded in ${loadTime}ms`);
      ...
      console.log(`📊 Branch sales inquiry data loaded successfully: ${barChartData.length} items for branch ${userBranch}`);

    } catch (error) {
      console.error('Error loading branch sales inquiry data:', error);
...
<section class="rounded-2xl sm:rounded-3xl bg-white/90 border border-white shadow-card p-4 sm:p-6 xl:p-5 2xl:p-6 h-[440px] sm:h-[550px] lg:h-[620px] xl:h-[540px] 2xl:h-[650px] overflow-hidden">
  <div class="flex flex-col gap-3 sm:gap-4">
    <div class="flex flex-row items-center justify-between gap-2 sm:gap-3 lg:gap-4">
      <div class="flex-1">
        <h2 class="text-base sm:text-lg lg:text-lg xl:text-lg font-bold text-slate-900 truncate">Sales & Inquiry Overview - {userBranch || 'Loading...'}</h2>
      </div>
